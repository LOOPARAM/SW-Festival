...
export default function HomeClient() {
  ...
  const [grade, setGrade] = useState<number>(() =>
    parseInt(searchParams.get("grade") ?? "3", 10)
  );
  const [semester, setSemester] = useState<number>(() =>
    parseInt(searchParams.get("semester") ?? "1", 10)
  );
  const [examType, setExamType] = useState<number>(() =>
    parseInt(searchParams.get("examType") ?? "1", 10)
  );
  ...
  }, [grade, semester, examType, router]);
          ...
          {[
            {
              ...
              //label: "연도 선택",
              value: year,
              options: [...Array(1)].map((_, i) => new Date().getFullYear() - i),
